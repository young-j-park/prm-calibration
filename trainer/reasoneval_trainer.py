...
class ReasonEvalBaseSFTTrainer(SFTTrainer):
    ...
    def compute_loss(self, model, inputs, num_items_in_batch=None, return_outputs=False):
        ...
        # ReasonEval does not explicitly use a step token.
        shift_logits = outputs[..., :-2, :].contiguous()
        shift_labels = inputs["labels"][..., 2:].contiguous()
...
class ReasonEvalMSESFTTrainer(ReasonEvalBaseSFTTrainer):
    ...
    def activate(self, logits: torch.Tensor) -> torch.Tensor:
        return logits.softmax(dim=-1)[:, 1:].sum(dim=-1)
...
class ReasonEvalQuantileSFTTrainer(ReasonEvalBaseSFTTrainer):
    def __init__(self, quantiles, candidate_token_ids, bin_token_ids, **training_args):
        """
